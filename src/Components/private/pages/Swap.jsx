...
function Swap() {
    ...
    const handleSwap = async () => {
        try {
            ...
            console.log(e)
            toast.error(e ?? "Failed to swap coin. Please try again later");
        } finally {
            setLoading(false)
        }
    }
        ...
        <div className="flex text-black gap-8 px-2 lg:px-8 py-8 w-full lg:flex-row flex-col">
            <div className="w-full lg:w-1/2 bg-white pt-8" ref={dropdownRef}>
                ...
                <div className="mt-6 mx-auto my-6 p-6 lg:pl-16 lg:pr-6 text-gray-500">
                    <button onClick={handleSwap} className="w-full bg-[#7044ee] text-white text-center py-4 rounded-md hover:bg-[#825fe9]">
                        {!loading ? 'Submit Swap' : 'Swapping...'}
                    </button>
                </div>
