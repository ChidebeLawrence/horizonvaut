...
function Investment() {
  ...
  const fetchInvestmentSchemes = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://api.horizonvaut.com/investment/schemes",
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        throw new Error(`Error fetching schemes (${response.status})`);
      }
    ...
    <div className="text-black gap-8 px-4 lg:px-8 py-12">
      <div className="bg-white rounded-2xl p-8">
        ...
        {closeModal && selectedPlan && (
          ...
          >
            <div className="relative bg-white text-black p-8 rounded-2xl w-[28rem] max-w-full sm:w-[32rem] md:w-[36rem] lg:w-[40rem] text-center shadow-2xl transition-all transform hover:scale-105">
              <button
